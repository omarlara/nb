...
Enbridge.ServiceUtils.loadProvinces = function (data, id, pickProvince) {
...
Enbridge.ServiceUtils.trackAccordionWizardStepsInWebTrends = function (e) {
  ...
  // Do not repeat step
  if (step > Enbridge.currentStep) {
    Enbridge.currentStep = step;
  }
  else {
    return;
  }
...
Enbridge.ServiceUtils.trackWebTrendsHandler = function () {
  var step = Enbridge.Plugins.AccordionWizard.getCurrentStep($('.accordion')),
      url = document.URL;
...
(function ($) {
  Enbridge.Plugins.Validator = function formValidator($form) {
    ...
    var $codeContainer;
    for (i = zipTool.length - 1; i >= 0; i-=1) {
      $current = $(zipTool[i]);
    ...
    for (i = newAddress.length - 1; i >= 0; i-=1) {
    ...
    for (i = select.length - 1; i >= 0; i-=1) {
      ...
      if ($current.attr('data-required') && !$current.val()) {
        ...
        if ('top' === $current.attr('data-position')) {
          $current
            .addClass('input-error')
    ...
    var _setRelDependenciesToInput =
        function(index, attr) {
    ...
    for (i = text.length - 1; i >= 0; i-=1) {
      ...
      if ($current.attr('data-required')) {
        ...
        $.each(attributes, _setRelDependenciesToInput);
      }
      if (!!$current.val()) {
        switch (pattern) {
          case 'postal-code':
    ...
    if (error) {
      for (i = oneFromGroup.length - 1; i >= 0; i -= 1) {
        $current = $(oneFromGroup[i]);
    ...
    if (!error) {
      ...
      var $item = '',
        isCheckable = true,
        dateItem = '';
      for (i = ageValidatorElements.length - 1; i >= 0; i -= 1) {
